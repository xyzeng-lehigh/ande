...
# Given the coefficient a_k, k >= -offset compute and plot the stability region of
#   E(z) = Re \sum_k coef[k+offset] exp(kz)
def plot_stab_exp_z(offset,coef,range_x,range_y,nsample):
  if nsample <= 0:
    nsample = 2001
  x = np.linspace(range_x[0],range_x[1],nsample)
  y = np.linspace(range_y[0],range_y[1],nsample)
  [X,Y] = np.meshgrid(x,y)
  W = np.zeros((nsample,nsample))
  l = offset
  r = len(coef)-1-l
  for k in range(-l,r+1):
    if k == 0:
...
# # Given the coefficient a_k, k >= -offset compute and plot the order star of
# #   sigma(z) = \sum_k coef[k+offset] exp(kz) - z
# def plot_stab_poly_z(offset,coef,range_x,range_y,nsample):
#   if nsample <= 0:
#     nsample = 2001
#   x = np.linspace(range_x[0],range_x[1],nsample)
#   y = np.linspace(range_y[0],range_y[1],nsample)
